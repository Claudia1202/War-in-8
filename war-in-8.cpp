...
int stanga,sus,width_tabla=560,height_tabla=560,latura, numar=8,k,window,window_w=800,window_h=560,comp,piesaeliminata,numarpieseeliminate,lin,window1;
int jucator=0;
bool randComp=false;
...
char limba[150][150]=
{
    "PLAYER VS COMPUTER", "PLAYER VS PLAYER", "SETTINGS", "HELP", "BACK", "LANGUAGE", "RESOLUTION","MUSIC","EXIT","COLORS","PLAYER 1 WON!", "PLAYER 2 WON!", "THE SCORE IS EQUAL!", "NEXT MOVE", "RED/YELLOW", "BLUE/GREEN", "MAGENTA/CYAN", "PLAYER 1", "PLAYER 2",
    "JUCATOR VS COMPUTER", "JUCATOR VS JUCATOR", "SETARI", "AJUTOR", "INAPOI","LIMBA", "REZOLUTIE","MUZICA", "IESIRE","CULORI","JUCATORUL 1 A CASTIGAT!", "JUCATORUL 2 A CASTIGAT!", "SCORUL ESTE EGAL!", "URMATOAREA MUTARE", "ROSU/GALBEN", "ALBAS/VERDE", "MAG/TURCOAZ", "JUCATOR 1", "JUCATOR 2",
    "JOUEUR VS ORDINATEUR", "JOUEUR VS JOUEUR", "REGLAGES GENERAUX","AIDER","ARRIERE","LANGUE","RESOLUTION","LA MUSIQUE","SORTIR","COULEURS","JOUEUR 1 A GAGNE!", "JOUEUR 2 A GAGNE!", "LE SCOR EST EGAL!", "SUIVANT MOUVEMENT", "ROUGE/JAUNE", "BLEU/VERT", "MAG/TURQOISE", "JOUEUR 1", "JOUEUR 2"
};
int nrLimba=0;
struct butoane
{
...
void mutarePiesa(int &jucator) 
{
    ...
    if(TablaDeJoc[linia1+1][coloana1+1]==0)
    {
    ...
    if(jucator==1) 
    {
        ...
        //outtextxy(width_tabla+40,maxy/2,"YELLOW");
    }
    else if(jucator==2)
    {
        if(comp==0)
        ...
        outtextxy(width_tabla+40,maxy/2+2,"LIGHTRED");*/
        setcolor(cul1);
...
void deseneazaPieseleInitiale()
{
    ...
    for (int i=1; i<=numar; i++)
        ...
        {
            TablaDeJoc[1][i]=1;
            deseneazaPiesa(1,i,culoare[1]);

            TablaDeJoc[7][i]=2;
            deseneazaPiesa(7,i,culoare[2]);
        }
        else
        {
            TablaDeJoc[numar][i]=2;
            deseneazaPiesa(numar,i,culoare[2]);

            TablaDeJoc[2][i]=1;
            deseneazaPiesa(2,i,culoare[1]);
...
{if(TablaDeJoc[i+1][j-1]!=8 and TablaDeJoc[i-1][j-1]!=8 and TablaDeJoc[i-1][j+1]!=8 and TablaDeJoc[i+1][j+1]==0)
    ...
    ok=1;}
                ...
                for(i=1;i<=8;i++)
        for(j=1;j<=8;j++){ if(TablaDeJoc[i][j]!=comp and TablaDeJoc[i][j]!=0 and ok==0){
...
///dace este incoltita de 3 piese si un colt liber se elibereaza
...
void joacaPVC_easy()
{
    ...
    while (nrMutari-1>0) ///-1 pt ca am mutat deja o piesa inainte de while
    {
        ...
        else if(randComp==true)
        {
            ...
            deseneazaPiesa(linieComp2,coloanaComp2,culoare[comp]);
            i++;
...
void stareMouse (int x1, int y1,int x2, int y2)
{
   ...
   // window1=initwindow(700, 560);
    ...
    while(1)
    {
     ...
     /*  if(ismouseclick(WM_LBUTTONDOWN))
        ...
        if(mx>=BackHelp.x1 && my>=BackHelp.y1 && mx<=BackHelp.x2&& my<=BackHelp.y2)
        {
            cleardevice();
            ///Functia de la meniu pt ca e declarata mai jos si nu o pot apela aici
            butoaneMeniu();
            while(1)
            {
                butoaneMeniuHover();
...
void PaginaDeSetari()
{
    ...
    while(1)
    {
        stareMouse(Back.x1,Back.y1,Back.x2,Back.y2);
        outtextxy(Back.x1,Back.y1,limba[nrLimba+4]);
        rectangle(Back.x1,Back.y1,Back.x2,Back.y2);

        stareMouse(exi2.x1,exi2.y1,exi2.x2,exi2.y2);
        outtextxy(exi2.x1,exi2.y1,limba[nrLimba+8]);
        rectangle(exi2.x1,exi2.y1,exi2.x2,exi2.y2);
        ///Highlight english
        stareMouse(language.x2+30,language.y1,language.x2+30+textwidth("ENGLISH"),language.y2);
        outtextxy(language.x2+30,language.y1,"ENGLISH");
        rectangle(language.x2+30,language.y1,language.x2+30+textwidth("ENGLISH"),language.y2);

        ///Highlight romana
        stareMouse(language.x2+30+textwidth("ENGLISH")+25,language.y1,language.x2+30+textwidth("ENGLISH")+25+textwidth("ROMANA"),language.y2);
        outtextxy(language.x2+30+textwidth("ENGLISH")+25,language.y1,"ROMANA");
        ...
        ///Highlight francais
        stareMouse(language.x2+30+textwidth("ENGLISH")+25+textwidth("ROMANA")+25,language.y1,language.x2+30+textwidth("ENGLISH")+25+textwidth("ROMANA")+25+textwidth("FRANCAIS"),language.y2);
        outtextxy(language.x2+30+textwidth("ENGLISH")+25+textwidth("ROMANA")+25,language.y1,"FRANCAIS");
        ...
        ///Highlight 800 x 560
        stareMouse(resolution.x2+20,resolution.y1,resolution.x2+20+textwidth("800 x 560"),resolution.y2);
        outtextxy(resolution.x2+20,resolution.y1,"800 x 560");
        rectangle(resolution.x2+20,resolution.y1,resolution.x2+20+textwidth("800 x 560"),resolution.y2);

        ///Highlight 1360 x 720
        stareMouse(resolution.x2+20+textwidth("800 x 560")+25,resolution.y1,resolution.x2+20+textwidth("800 x 560")+25+textwidth("1360 x 700"),resolution.y2);
        outtextxy(resolution.x2+20+textwidth("800 x 560")+25,resolution.y1,"1360 x 700");
        ...
        else if(my>=colors.y1 && my<=colors.y2)
        {

        }
        else if(my>=language.y1 && my<=language.y2)///Language
        {
...
int main()
{
    ...
    int k;
    for(k=0; k<=9; k++)
        TablaDeJoc[0][k]=TablaDeJoc[9][k]=8;
    for(k=0; k<=9; k++)
